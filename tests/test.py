...
from ocdutils import dtnamer, filesystem


class DatetimeTest(unittest.TestCase):
    def test_name_set_op(self):
        x = dtnamer.NameHandler(format="foo-%Y%m%d.%H%M%S")
        dt = datetime(year=1992, month=1, day=2, hour=3, minute=4, second=5)
    ...
    def test_exif_set_op_with_zero(self):
        x = dtnamer.ExifHandler()
        dt = datetime(year=1992, month=1, day=2, hour=3, minute=4, second=0)
